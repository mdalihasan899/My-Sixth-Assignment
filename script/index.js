...
const loadPlants = (id) => {
  ...
  fetch(`https://openapi.programming-hero.com/api/category/${id}`)
    .then((res) => res.json())
    .then((json) => displayDifPlants(json.plants))
}
...
const displayDifPlants = (DifPlants) => {
  ...
  DifPlants.forEach(DifPlant => { 
    ...
    <div class="max-w-sm bg-white rounded-lg shadow p-4 items-center">
            <img class="w-full h-40 rounded-md mb-4" src="${DifPlant.image}" alt="">
...
const displayCategories = (categories) => {
  ...
  for (const categorie of categories) {
    ...
    categorieBtn.innerHTML = `
        <button onclick="loadPlants(${categorie.id})" href=""
            class="hover:bg-[#15803D] hover:scale-105 hover:text-white w-full text-start py-2 pl-[10px] rounded-[4px] mt-1">${categorie.category_name}</button>
        `
  ...
  for (const Tree of Trees) {
    ...
    TreeCard.innerHTML = `
        <div class="max-w-sm bg-white rounded-lg shadow p-4 h-full items-center">
            <img class="w-full h-40 rounded-md mb-4" src="${Tree.image}" alt="">
